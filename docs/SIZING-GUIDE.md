...
**Snapshot:** CPU/memory figures were taken from kpt (`feat/ip-setters`) and nok-clabs manifests as of 2026-09. Empty cells (`—`) mean no request/limit is set in the manifest, not missing data.
...
Disable at deploy time with `SDCIO_ENABLED=NO` to drop platform SDCIO + recipe exporters.
...
| Prometheus PVC (BBM) | `10Gi` | _(kpt default; not tunable via Makefile yet)_ |
